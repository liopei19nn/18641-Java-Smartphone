...
//						e.printStackTrace();
				...
				else if (strInput.equals("2")){//2 if you get configure request 
				...
				// 3 handle servlet select car model request
				else if (strInput.equals("3")){
					// 3.1 get all auto name in server and send back
					ArrayList<String> autoNameList = buildCarModelOptions.getAllModelList();
					try {
						objectOutputStream.writeObject(autoNameList);
				...
				// 4 handle servlet get an automobile request
				else if (strInput.equals("4")){
					// 4.1 get model name from server
					String modelName = null;
					try {
						modelName = (String)objectInputStream.readObject();
					} catch (ClassNotFoundException | IOException e) {
					...
					// 4.2 send the selected auto mobile to client
					try {
						buildCarModelOptions.sendSelectedModel(objectOutputStream, modelName);
					} catch (IOException e1) {
						e1.printStackTrace();
